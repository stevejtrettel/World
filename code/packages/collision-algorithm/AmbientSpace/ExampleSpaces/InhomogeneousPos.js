...
//this one is an INCOMPLETE METRIC and is only defined on a sphere about the origin of radius
...
function sAux(x){
    return Math.asin(x) + x*Math.sqrt(1-x*x)/2;
}
...
//maximum radius of the geometry is 1; where metric breaks down
let coordSize = 0.85;
let sphereSize = sAux(coordSize);
...
let inhomogeneousPos = new AmbientSpace( space, model, obstacle);

export { inhomogeneousPos };
