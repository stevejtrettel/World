...
//THIS EXAMPLE TRIES OUT A NEW FUNCTION PARSER
//BUT ITS SLOW :(
import Formula from "../../../../3party/fparser.js";
...
class RiemannSum2DFubini {
    constructor(setup=defaultSetup){
       ...
       // let func = parser.evaluate('f(x,y,t,a,b,c)='.concat(this.params.functionText));
        let funcNEW = new Formula(this.params.functionText);

        //the function with all the variables:
        this.f = function(coords, params={time:0,a:0,b:0,c:0}){
            let z = funcNEW.evaluate({x: coords.x, y:coords.y,t:params.time, a:params.a,b:params.b,c:params.c});
    ...
    addToUI(ui){
        ...
        ui.add(this.params,'functionText').name('z=').onFinishChange(
            function(value){
                ...
                let curveNEW = new Formula(value);
                let eqn = function(coords ,params={time:0,a:0,b:0,c:0}){
                    let z = curveNEW.evaluate({x: coords.x, y:coords.y,t:params.time, a:params.a,b:params.b,c:params.c});
                    return z;
                }
