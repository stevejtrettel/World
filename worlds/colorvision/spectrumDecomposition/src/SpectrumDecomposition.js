...
let simpleCurve = function(f){
    return  2.*Math.exp(-15.*(f-0.5)*(f-0.5))+0.5;
}
...
class SpectrumDecomposition{
    constructor(params=defaultParams){
        ...
        this.xAxis = new Rod({
            end1: new Vector3(-5,0,0),
            end2: new Vector3(5,0,0),
            color: 0x000000,
            radius:0.05,
        });
        this.xAxis.setPosition(0,-2,0);

        this.yAxis = new Rod({
            end1: new Vector3(-5,0,0),
            end2: new Vector3(-5,3,0),
            color: 0x000000,
            radius:0.05,
        });
        this.yAxis.setPosition(0,-2,0);
    }
    ...
    tick(time,dTime){
        ...
        this.currentPos.setColor(spectralColor((1+Math.sin(time/3))/2.));
